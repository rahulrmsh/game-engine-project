...
void Sandbox2D::OnUpdate(GammaEngine::Timestep ts)
{
...

	{
		...
		GammaEngine::Renderer2D::DrawRotatedQuad({ 1.0f, 0.0f }, { 0.8f, 0.8f }, glm::radians(- 45.0f), {0.8f, 0.2f, 0.3f, 1.0f});
		...
		GammaEngine::Renderer2D::DrawRotatedQuad({ -2.0f, 0.0f, 0.0f }, { 1.0f, 1.0f }, glm::radians(rotation), m_CheckerboardTexture, 20.0f);
	...
	if (GammaEngine::Input::IsMouseButtonPressed(GAMMAENGINE_MOUSE_BUTTON_LEFT))
	{
		auto [x, y] = GammaEngine::Input::GetMousePosition();
		auto width = GammaEngine::Application::Get().GetWindow().GetWidth();
		auto height = GammaEngine::Application::Get().GetWindow().GetHeight();
